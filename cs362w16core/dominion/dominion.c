...
int minion_card(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2)
{
  int i;
  int j;
...
int embargo_card(int currentPlayer, struct gameState *state, int handPos, int choice1)
{
  //+2 Coins
  state->coins = state->coins + 2;
...
int salvager_card(int currentPlayer, struct gameState *state, int handPos, int choice1)
{
  //+1 buy
  state->numBuys++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventurer_card(currentPlayer, state);
  /* ---------refactored-----------
      while(drawntreasure<2){
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
    ...
    case minion:
      minion_card(currentPlayer, state, handPos, choice1, choice2);
    /*-------refactored----------
      //+1 action
      state->numActions++;
    ...
    case embargo: 
      embargo_card(currentPlayer, state, handPos, choice1);
      /*--------refactored------------
      //+2 Coins
      state->coins = state->coins + 2;
    ...
    case salvager:
      salvager_card(currentPlayer, state, handPos, choice1);
      /*--------refactored---------------
      //+1 buy
      state->numBuys++;
